...
import Filter, { MyFilterProps } from "./Filter";
...
function HeaderDefaultContent({category, changeCategory, changeSearchValue, filterClassName, searchBarIsFocused, setSearchBarIsFocused, clearSearchBtnFocused, setClearSearchBtnFocused}: MyFilterProps): ReactJsxElm {
    return (
        <div className="header_default">
            <LogoIcon className="header_logo-icon" mainColor={appColors["extra-light-color"]}/>
            <div className="header_info-group">
                <a href="#aboutus">About Us</a>
                <a href="#contacts">Contacts</a>
                <button className="profile-picture"><img src="https://avatars.githubusercontent.com/u/71090230?s=400&u=a0e3cf64f7329d3bbad75547e25b67724e0d10c7&v=4" alt="account picrture" /></button>
            </div>
            <Filter filterClassName={filterClassName} category={category} changeCategory={changeCategory} changeSearchValue={changeSearchValue} searchBarIsFocused={searchBarIsFocused} setSearchBarIsFocused={setSearchBarIsFocused} clearSearchBtnFocused={clearSearchBtnFocused} setClearSearchBtnFocused={setClearSearchBtnFocused}/>
        </div>
    )
};
...
export default function Header({ category, changeCategory, changeSearchValue, searchValue} : MyHeaderProps): ReactJsxElm {
    ...
    const [headerContent, setHeaderContent] = useState(<HeaderDefaultContent />)
...
{/* 
            ...
            {/* <Filter className={`header_filter ${headerIsFixed ? "header--fixed_filter" : "header--not-fixed_filter"}`} category={category} changeCategory={changeCategory} changeSearchValue={changeSearchValue} searchBarIsFocused={searchBarIsFocused} setSearchBarIsFocused={setSearchBarIsFocused} clearSearchBtnFocused={clearSearchBtnFocused} setClearSearchBtnFocused={setClearSearchBtnFocused}/> */}
            
            {/* <HeaderDefaultContent filterClassName={headerIsFixed ? "header--fixed" : "header--default"} category={category} changeCategory={changeCategory} changeSearchValue={changeSearchValue} searchBarIsFocused={searchBarIsFocused} setSearchBarIsFocused={setSearchBarIsFocused} clearSearchBtnFocused={clearSearchBtnFocused} setClearSearchBtnFocused={setClearSearchBtnFocused}/> */}
            {headerContent}
        </header>        
    )
}